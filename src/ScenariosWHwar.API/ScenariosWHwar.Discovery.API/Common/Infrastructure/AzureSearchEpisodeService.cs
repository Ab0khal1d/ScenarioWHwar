...
namespace ScenariosWHwar.Discovery.API.Common.Infrastructure;

/// <summary>
/// Configuration options for Azure Cognitive Search service.
/// </summary>
public class AzureSearchOptions
{
    ...
    /// <summary>
    /// Gets the search service endpoint URL.
    /// </summary>
    public string ServiceEndpoint => $"https://{ServiceName}.search.windows.net";
}
...
public class AzureSearchEpisodeService : IEpisodeSearchService
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var searchOptions = new SearchOptions
            {
                ...
                OrderBy = { "publishDate desc" } // Order by publish date descending
            };
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var searchOptions = new SearchOptions
            {
                Filter = $"id eq {id} and status eq 'Ready'",
                Size = 1
            };
    ...
    private static string BuildFilterExpression(EpisodeSearchFilter filter)
    {
        ...
        {
            "status eq 'Ready'", // Only show ready episodes
            $"publishDate le {DateTime.UtcNow:yyyy-MM-ddTHH:mm:ssZ}" // Only show published episodes
        };

        if (!string.IsNullOrWhiteSpace(filter.Category))
        {
            filters.Add($"category eq '{filter.Category.Replace("'", "''")}'");
        }

        if (!string.IsNullOrWhiteSpace(filter.Language))
        {
            filters.Add($"language eq '{filter.Language.Replace("'", "''")}'");
        }
