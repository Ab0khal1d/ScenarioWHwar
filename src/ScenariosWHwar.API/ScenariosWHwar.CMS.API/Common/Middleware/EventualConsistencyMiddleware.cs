using ScenariosWHwar.API.Core.Common.Domain.Base.EventualConsistency;
using ScenariosWHwar.API.Core.Common.Domain.Base.Interfaces;

namespace ScenariosWHwar.CMS.API.Common.Middleware;
...
public class EventualConsistencyMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context, IPublisher publisher, ApplicationDbContext dbContext)
    {
        context.Response.OnCompleted(async () =>
        {
            ...
            await strategy.ExecuteInTransactionAsync(async () => await PublishEvents(context, publisher), null!);
        });
